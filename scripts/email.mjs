...
export async function deliverEmail(state,env=process.env,send=fetch){
 const configured=!!(env.RESEND_API_KEY&&env.ALERT_EMAIL_FROM&&env.ALERT_EMAIL_TO);
 ...
 return {configured:true,pending,deliveryError};
}
